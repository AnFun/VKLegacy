...
public enum ParticleEffect {
  ...
  WATER_BUBBLE(
    ...
    "bubble", 4, -1, new ParticleProperty[] { ParticleProperty.DIRECTIONAL, 
  ...
  ITEM_CRACK(
    ...
    "iconcrack", 36, -1, new ParticleProperty[] { ParticleProperty.DIRECTIONAL, 
  ...
  static {
    ...
    for (i = (arrayOfParticleEffect = values()).length, b = 0; b < i; ) {
      ParticleEffect effect = arrayOfParticleEffect[b];
  ...
  public void display(Vector direction, float speed, Location center, double range) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
    ...
    if (hasProperty(ParticleProperty.REQUIRES_WATER) && !isWater(center))
      throw new IllegalArgumentException(
          "There is no water at the center location"); 
    (new ParticlePacket(this, direction, speed, (range > 256.0D), null)).sendTo(
        center, range);
  }
  ...
  public void display(ParticleData data, Vector direction, float speed, Location center, double range) throws ParticleVersionException, ParticleDataException {
    ...
    if (!isDataCorrect(this, data))
      throw new ParticleDataException(
          "The particle data type is incorrect"); 
    (new ParticlePacket(this, direction, speed, (range > 256.0D), data)).sendTo(
        center, range);
  }
  ...
  public static final class ParticlePacket {
    ...
    public static void initialize() throws VersionIncompatibleException {
      ...
      try {
        ...
        getHandle = ReflectionUtils.getMethod("CraftPlayer", 
            ReflectionUtils.PackageType.CRAFTBUKKIT_ENTITY, "getHandle", new Class[0]);
        playerConnection = 
          ReflectionUtils.getField("EntityPlayer", ReflectionUtils.PackageType.MINECRAFT_SERVER, 
            false, "playerConnection");
        sendPacket = ReflectionUtils.getMethod(
            playerConnection.getType(), "sendPacket", new Class[] { ReflectionUtils.PackageType.MINECRAFT_SERVER.getClass("Packet") });
    ...
    public void sendTo(Location center, Player player) throws PacketInstantiationException, PacketSendingException {
      ...
      try {
        sendPacket.invoke(
            playerConnection.get(getHandle.invoke(player, new Object[0])), new Object[] { this.packet });
      } catch (Exception exception) {
