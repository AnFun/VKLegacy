...
public class Banks implements Listener {
    public static HashMap<Player, String> banksee = new HashMap<>();
    ...
    public void saveBank(Inventory inv, Player p) {
        ...
        if (banksee.containsKey(p))
            name = banksee.get(p);
        File file = new File(Main.plugin.getDataFolder() + "/banks", String.valueOf(name) + ".yml");
    ...
    public void onInvOpen(InventoryOpenEvent e) {
        if (e.getInventory().getName().equals("Bank Chest (1/1)")) {
            ...
            im.setLore(Arrays.asList(new String[] { ChatColor.GRAY + "Right Click to create " + ChatColor.GREEN + "A GEM NOTE" }));
            gem.setItemMeta(im);
            inv.setItem(58, gem);
        }
    }
        ...
        if (this.withdraw.contains(p.getName())) {
            ...
            try {
                ...
                if (amt > Main.econ.getBalance((OfflinePlayer)p)) {
                    ...
                    for (int i = 0; i < this.withdraw.size(); i++)
                        this.withdraw.remove(p.getName());
                    Main.econ.withdrawPlayer((OfflinePlayer)p, amt);
                    ...
                    p.sendMessage(ChatColor.GREEN + ChatColor.BOLD + "New Balance: " + ChatColor.GREEN +
                            (int)Main.econ.getBalance((OfflinePlayer)p) + " GEM(s)");
    ...
    public void onInvClick(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem() != null && e.getCurrentItem().getType() == Material.PAPER &&
            ...
            im.setLore(Arrays.asList(new String[] { ChatColor.WHITE + ChatColor.BOLD + "Value: " + ChatColor.WHITE + (first + second) + " Gems",
